...
function init() {
  ...
  var teapotGeometry = new THREE.TeapotGeometry(
    1,
    15,
    true,
    true,
  ...
  saturnGroup.position.set(-20, 0, 10);
  solar.add(saturnGroup);
  ...
  torusGeometry = new THREE.TorusGeometry(4, 0.5, 64, 32);
  var torus = new THREE.Mesh(
    torusGeometry,
    new THREE.MeshBasicMaterial({ color: "beige" })
  );
  saturnGroup.add(torus);
  // squish it (scale)
  ...
  var camera_axis = new THREE.Vector3(-30, 30, 0).normalize(); // viewing axis
  camera.camera_axis = camera_axis;
